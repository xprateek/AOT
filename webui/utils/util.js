...
export const basePath = "/data/adb/aot";
export const moduleDirectory = "/data/adb/modules/aot-tethering";
...
export function linkRedirect(link) {
    ...
    setTimeout(() => {
        if (typeof $aot !== 'undefined' && Object.keys($aot).length > 0) {
...
export async function checkMMRL() {
    if (typeof $aot !== 'undefined' && Object.keys($aot).length > 0) {
        ...
        try {
            $aot.setLightStatusBars(!window.matchMedia('(prefers-color-scheme: dark)').matches)
        } catch (error) {
